...
export default function SeatMapGallery() {
  ...
  return (
    <section className="seat-guide-panel">
      <header>
        <h2>항공기별 좌석 배치도</h2>
        <p>기종을 선택해 좌석 위치를 확인하세요. 실제 운항 기종은 변경될 수 있습니다.</p>
      </header>

      <div className="seat-map-tabs" aria-label="항공기 기종">
        {SEAT_MAPS.map((map) => (
          <button
            type="button"
            aria-pressed={activeMapId === map.id}
            className={activeMapId === map.id ? 'is-active' : ''}
            key={map.id}
      ...
      <div className="seat-map-list">
        <figure>
          <figcaption>{activeMap.label}</figcaption>
          <div className="seat-map-image">
            <img src={activeMap.src} alt={`${activeMap.label} 좌석 배치도`} />
          </div>
        </figure>
      </div>
    </section>
